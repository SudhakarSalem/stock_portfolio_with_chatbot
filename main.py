...
def ask_gemini(user_message: str) -> str:
    ...
    system_instruction = (
        "You are a financial assistant that only financial news  include current portfolio holdings. no other questions are answered"
...
async def index():
    """Always respond at '/' — with the UI if found, or a clear diagnostic if not."""
    ...
    return {
        ...
        "hint": (
            "Make sure a 'static' folder containing index.html sits in the "
            "same directory as main.py, then restart the server."
        ),
        "api_docs": "/docs",
    }
